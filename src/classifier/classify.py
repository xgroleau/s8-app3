...
def display_confusion_matrix(labels, expected_labels, fitted_labels, normalize="true"):
    ...
    if len(aggregated_labels) < len(labels):
        ...
        confusion_matrix2 = metrics.confusion_matrix(expected_labels, fitted_labels, labels=aggregated_labels,
                                                    normalize=normalize)

        display2 = metrics.ConfusionMatrixDisplay(confusion_matrix2, display_labels=aggregated_labels)
        display2.plot()
